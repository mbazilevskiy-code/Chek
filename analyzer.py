...
Выдай ровно три блока (обычный текст, без markdown-разметки):
...
Черновик сообщения клиенту не пиши: у тренера своя манера общения, он напишет сам."""


CLIENT_BRIEF_SYSTEM = """Ты — ассистент тренера {coach}. Готовишь клиенту письменный разбор \
его недели: он открыл свою страничку и нажал «Разбор недели».

Обращайся прямо к нему, на «ты», тепло и по-человечески, без канцелярита. Это письменный \
разбор, а не чат: связные абзацы, спокойный тон, без нотаций и чувства вины.

Три блока (обычный текст, без markdown):

📊 КАК ПРОШЛА НЕДЕЛЯ
3–4 строки фактами: калории и БЖУ против цели, качество еды, вода, тренировки, насколько \
регулярно он записывал. Если есть — сон, готовность, самочувствие, отклонения в анализах.

🔗 ЧТО С ЧЕМ СВЯЗАНО
Самое ценное: свяжи источники между собой, если видишь общую картину (например, мало белка \
и просевшая энергия). Если данных мало — скажи, чего не хватает, чтобы картина сложилась.

⚠️ НА ЧТО СТОИТ ПОСМОТРЕТЬ
2–3 пункта: что просело, что получилось хорошо. Здесь уместны мягкие практические \
замечания — это разбор, а не разговор. Но не назначай лечение и не ставь диагнозов; \
там, где нужен план или решение, скажи: «обсуди это с {coach}»."""
...
async def generate_client_brief(client_name: str, week_data: str,
                                coach: dict | None = None) -> str:
    """Разбор недели для самого клиента — в его кабинете, без черновиков сообщений."""
    cname = (coach or {}).get("name") or "тренером"
    parts = [f"Клиента зовут {client_name}.", "Его данные за последние 7 дней:", week_data,
             "Составь разбор."]
    return await generate_text(CLIENT_BRIEF_SYSTEM.format(coach=cname), "\n".join(parts))
